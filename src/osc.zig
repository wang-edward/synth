...
pub const OscConfig = struct {
    ...
    pub fn process(self: *OscConfig, t: f32) f32 {
        ...
        return switch (self.osc_type) {
            OscType.sine => std.math.sin(t * radians_per_second),
            OscType.square => if (normalized_time < 0.5) 1.0 else -1.0,
            OscType.triangle => 4.0 * @abs(normalized_time - 0.5) - 1.0,
            OscType.saw => 2.0 * normalized_time - 1.0,
        };
    }
};
